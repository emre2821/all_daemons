...
def setup_paths(eden_root=None):
    """Setup paths based on provided eden_root or environment detection"""

    if eden_root is None:
        eden_root = os.environ.get("EDEN_ROOT")

    if eden_root is None:
        # Try to detect Eden root relative to current script
        current_dir = Path(__file__).parent
        possible_roots = [
            current_dir.parent.parent.parent.parent,  # Go up from Red_Thread_Pipeline/Dave
        ...
        for root in possible_roots:
            if (root / "all_daemons").exists() or (root / "data").exists():
                eden_root = str(root)
                break

        # Fallback
        if eden_root is None:
            eden_root = str(current_dir.parent.parent)

    eden_root = Path(eden_root)
...
# Setup paths with detection
paths = setup_paths()
EDEN_ROOT = paths['eden_root']
...
def scan_logs():
    ...
    details = defaultdict(
        lambda: {"errors": 0, "warns": 0, "lines": 0, "files": 0, "bytes": 0}
    )
    ...
    for sub in OUTBOX.iterdir():
        ...
        scores[name] = fcount * 2 + int(bcount / 50000)  # ~2 pts/file + 1 per 50KB
...
def write_outputs(scores, details):
    ...
    ALERTS_JSON.write_text(
        json.dumps({"threshold": threshold, "ops": ops}, indent=2), encoding="utf-8"
    )
