...
class SmartRecover:
    ...
    def build_plan(self) -> 'Plan':
        ...
        for token, paths in self.groups.items():
            cat_counts = Counter(decide_category(p) for p in paths)
...
def decide_category(p: Path) -> str:
    ext = p.suffix.lower()
    if ext in EDENISH_EXTS: return "Eden"
    if ext in CODE_EXTS: return "Code"
    if ext in DOC_EXTS: return "Docs"
    if ext in IMG_EXTS: return "MediaImages"
    if ext in AUDIO_EXTS: return "MediaAudio"
...
class EdenRecoveryUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Toolbar
        toolbar = BoxLayout(size_hint_y=None, height=40, spacing=6)
        toolbar.add_widget(Button(text="Scan", on_release=lambda *_: self.scan_now()))
        toolbar.add_widget(Button(text="Preview Plan", on_release=lambda *_: self.preview_plan()))
        toolbar.add_widget(Button(text="Apply Moves", on_release=lambda *_: self.apply_moves()))
        toolbar.add_widget(Button(text="Debug", on_release=lambda *_: self.debug_state()))
        self.add_widget(toolbar)

        # Split area (Tree + File list)
        split = BoxLayout()
        self.tree = TreeView(hide_root=True)
        self.tree.add_node(TreeViewLabel(text="Root"))
        split.add_widget(self.tree)
        self.file_list = FileChooserListView(filters=["*.*"])
        split.add_widget(self.file_list)
        ...
        # Internals
        default_root = Path(os.getcwd()) / "5_deployments"
        self.root_folders = [default_root]
        self.rhea_input = str(default_root.parent / "CHAOS_Logs")
        self._plan = None
        self._rec = None
    ...
    def _scan_work(self):
        try:
            roots = [self.root_folders[0]]
            rhea = Path(self.rhea_input).resolve()
            self.log(f"Scanning {len(roots)} folder(s)...")
            rec = SmartRecover(roots, rhea).scan().categorise().detect_groups().compute_signals()
            self._rec = rec
            self._plan = rec.build_plan()
    ...
    def apply_moves(self):
        if not self._plan:
            ...
            return
        content = BoxLayout(orientation='vertical', spacing=6, padding=6)
        content.add_widget(Label(text="Move files according to plan?"))
        btns = BoxLayout(spacing=6, size_hint_y=None, height=48)
        nah_btn = Button(text="Nah")
        yeah_btn = Button(text="Yeah")
        btns.add_widget(nah_btn)
        btns.add_widget(yeah_btn)
        content.add_widget(btns)
        popup = Popup(title="Apply Moves?", content=content, size_hint=(0.6,0.3))
        nah_btn.bind(on_release=lambda *_: popup.dismiss())
        yeah_btn.bind(on_release=lambda *_: self._do_apply(popup))
        popup.open()
