...
# - Watches ./all_daemons for changes
...
DAEMONS_ROOT = RHEA_DIR.parent                     # .../all_daemons
...
def discover_daemons() -> Dict[str, Dict[str, Any]]:
    ...
    for pyfile in DIR_DAEMONS.rglob("*.py"):
        ...
        discovered[daemon_name] = {
            "name": daemon_name,
            # Store path relative to the daemons root (all_daemons)
...
@APP.command()
def scan():
    """Scan all_daemons and reconcile into registry."""
    _ensure_dirs()
    discovered = discover_daemons()
    reg = _load_registry()
    reg, logs = reconcile_registry(reg, discovered)
