...
3) ACTIVATE: if full_rhea.complete_build.py exists, init/scan/fix and start daemons
             ...
             otherwise, fallback: run every *.py under all_daemons/** as a process
...
# The Eden "all_daemons" root is the parent of Rhea
...
DEFAULT_RULES = {
    "version": 1,
    "targets": {
        "py_daemon_default": "all_daemons/Unsorted",
...
def plan_repairs(root: Path, rules: Dict) -> List[MoveOp]:
    ...
    for p in root.rglob("*"):
        ...
        if "all_daemons" in p.parts:
            continue

        ext = p.suffix
        bucket = ext_bucket(ext, rules)
        if bucket == "daemon_py" and ext == ".py":
            dst_base = ROOT / targets.get("py_daemon_default", "all_daemons/Unsorted")
        elif bucket == "chaos":
...
def start_without_rhea(parallel: int) -> int:
    # Fallback: run every python file under all_daemons/** directly
    files = [p for p in DIR_DAEMONS.rglob("*.py")]
    if not files:
        C.print("No daemons found under all_daemons/", style="yellow" if RICH else None)
        return 0
