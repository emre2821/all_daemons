    ...
    def __init__(self, root=None):
        ...
        if root is None:
            ...
            try:
                ...
                sys.path.insert(0, str(repo_root / "shared" / "Daemon_tools" / "scripts"))
                from eden_paths import daemons_root
                self.root = daemons_root()
            except ImportError:
                # Fallback to environment variable or current directory
                self.root = Path(os.environ.get("EDEN_ROOT", Path.cwd())) / "daemons"
        else:
            self.root = Path(root)
        self.rules = {
            "logs": ["*.log"],
