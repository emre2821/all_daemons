...
What this does: - Auto-discovers sibling daemons in 01_Daemon_Core_Agents\* - Dynamically imports each daemon module and queries:
        describe() -> str | dict (optional)
        healthcheck() -> {"status": "ok"/"warn"/"fail", ...} (optional)
        run(payload|path, **kwargs) -> any (optional) - Maintains/merges rhea_registry.json (non-destructive; creates .bak backup) - Provides a small CLI for: health map, fixing Sheele input, running Sheele->Briar->Codexa->Janvier->Aderyn - Safe defaults for OpenAI export path

Minimal external deps: - None required. - Optional: jsonschema (pip install jsonschema) if you enable schema checks.
"""
