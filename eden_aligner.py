#!/usr/bin/env python3
"""
eden_aligner.py — Organizer & healer for C:\\EdenOS_Origin\\all_daemons
...
# --------- Defaults ---------
DEFAULT_EDEN_ROOT = Path(os.environ.get("EDEN_ROOT", r"C:/EdenOS_Origin"))
...
def ensure_standard_dirs(daemon_dir: Path, apply: bool) -> List[Path]:
    created = []
    for sub in STANDARD_SUBDIRS:
        target = daemon_dir / sub
...
def run_saphira_heal(eden_root: Path, audit: bool, apply: bool) -> None:
    try:
        ...
        saphira_path = eden_root / "all_daemons" / "Saphira" / "saphira.py"
        ...
        sync = SaphiraSynchronizer(str(eden_root / "all_daemons"), logger_func=log)
...
def align_all(eden_root: Path, apply: bool, patch_code: bool, aggressive: bool, heal: bool) -> None:
    all_daemons = eden_root / "all_daemons"
    if not all_daemons.exists():
        raise SystemExit(f"Missing all_daemons at {all_daemons}")

    log(f"Scanning {all_daemons} …")
    for daemon_dir in sorted([p for p in all_daemons.iterdir() if p.is_dir()]):
        ...
        created = ensure_standard_dirs(daemon_dir, apply)
        if created:
...
def main():
    ap = argparse.ArgumentParser(description="EdenOS all_daemons organizer & healer")
    ap.add_argument("--root", default=str(DEFAULT_EDEN_ROOT), help="EDEN_ROOT (default: env EDEN_ROOT or C:/EdenOS_Origin)")
    ...
    align_all(eden_root, apply=args.apply, patch_code=args.patch_code, aggressive=args.aggressive, heal=args.heal)
