...
- Uses EDEN_ROOT from env or defaults to C:\EdenOS_Origin on Windows, ~/EdenOS_Origin elsewhere.
- Places Rhea under: <EDEN_ROOT>/all_daemons/Rhea
...
WIN_DEFAULT = r"C:\EdenOS_Origin"
NIX_DEFAULT = str(Path.home() / "EdenOS_Origin")
...
- all_daemons/Rhea : Rhea daemon home
...
- all_daemons/Rhea/rheagui.py      : minimal GUI launcher (replace with your full GUI)
- all_daemons/Rhea/rhea.py         : minimal core class
- all_daemons/Rhea/scriptum.py     : non-interactive journaling lib
- all_daemons/Rhea/rhea_gui_settings.json : settings w/ eden_root and theme stub
...
setlocal
set ROOT=%~1
if "%ROOT%"=="" set ROOT=C:\EdenOS_Origin

if not exist "%ROOT%" mkdir "%ROOT%"
if not exist "%ROOT%\all_daemons" mkdir "%ROOT%\all_daemons"
if not exist "%ROOT%\all_daemons\Rhea" mkdir "%ROOT%\all_daemons\Rhea"
...
ROOT="${1:-$HOME/EdenOS_Origin}"
mkdir -p "$ROOT" \
         "$ROOT/all_daemons/Rhea" \
         "$ROOT/logs" \
         "$ROOT/config" \
         "$ROOT/bin"
echo "Dirs ensured under $ROOT"
...
def ensure_dirs(root: Path):
    ...
    (root / "all_daemons").mkdir(parents=True, exist_ok=True)
    (root / "all_daemons" / "Rhea").mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    parser.add_argument("--eden-root", default=os.environ.get("EDEN_ROOT") or (WIN_DEFAULT if os.name == "nt" else NIX_DEFAULT),
    ...
    rhea_dir = eden_root / "all_daemons" / "Rhea"
    ...
    print(f"\nEden root: {eden_root}")
    print("\nNext:")
    print("  1) Put your full-featured rheagui.py in all_daemons/Rhea (replace scaffold).")
    print("  2) Run GUI:  python all_daemons/Rhea/rheagui.py")
